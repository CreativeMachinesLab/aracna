...
class Robot():
    ...
    def initServos(self, possibilities = None):
        # Set your serial port accordingly.
        if os.name == "posix":
            if possibilities is None: possibilities = ['/dev/ttyUSB0', '/dev/ttyUSB1']
            elif not isinstance(possibilities, list): possibilities = [possibilities] 
            portName = None
...
#
    ...
    def relax(self):
        '''Relaxes all the servos so that the robot is now posable'''
        for actuator in self.actuators:
            actuator.torque_enable = False
        self.net.synchronize()
        #for actuator in self.actuators:
        #    print "Torque status " + str(actuator.id) + ": " + str(actuator.torque_enable)
    
    def interpCommand(self, goalPos, duration):
        '''Commands movement to the given position over a specified duration (ms)'''
        
        # Figure out how fast to move each servo by interpolation
        curPos = self.readCurrentPosition()
        for i in range(len(self.actuators)):
            actuator = self.actuators[i]
            speed = RPM_CONVERSION_FACTOR * abs(goalPos[i]-curPos[i])/float(duration)
            if speed > 1023: actuator.moving_speed = 1023
            elif speed < 1: actuator.moving_speed = 1
            else: actuator.moving_speed = int(speed)
            
            
            actuator.torque_enable = True
            actuator.goal_position = goalPos[i]
